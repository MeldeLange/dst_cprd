*Analysis 6: Extract datasets needed for main regression analyses
*****************************************************************


*1. Combine pratice files
****************************

*1a) Loop to open the 5 practice files, save practice id & region.
clear
...
foreach f in `r(files)' {
	use "`f'", clear
	keep  pracid region // keep practice id, up to standard date, last collection date
	save "region/`f'", replace
			}
			
*1b) Loop to append the 5 temporary practice files
clear
...
*2. Merge region data with eventlist
*************************************
*2a)Create region variable in eventlist
...
save eventlist_`outcome'_19.dta, replace
}

*2b)Merge region data with eventlist
cd "projectnumber\cprd_data\gold_primary_care_all\stata\region"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
use practice.dta, clear
merge 1:m pracid using "projectnumber\cprd_data\combined\combined_eventlists/eventlist_`outcome'_19.dta"
drop _merge
save "projectnumber\cprd_data\combined\combined_eventlists/eventlist_`outcome'_20.dta", replace
}
************************

*3. Extract the datasets, then combine year & region data for each outcome / season.
***********************************************************************************************************************************
...
*Save datasets
save "projectnumber\analysis\datasets\comparisons/region/`outcome'_`season'_`year'_`region'.dta", replace
}
}
...
*3b) Append the collapsed datasets so all regions in the same year are together for each outcome / season.
...
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach season in Spring Autumn{
			foreach num of numlist 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019{
	capture{
		use "`outcome'_`season'_`num'_1.dta", clear
		foreach region in 2 3 4 5 6 7 8 9{
		append using "`outcome'_`season'_`num'_`region'.dta"
		}
		save "`outcome'_`season'_`num'_allregions.dta", replace		
	}		
...
*Do eating disorders spring 2013 & 2015 manually (don't have data for all regions) eat dis spring 2013 doesn't have region4. eatdis spring 2015 doesn't have region1.
clear
use eatdis_Spring_2013_1.dta, clear
foreach region in 2 3 5 6 7 8 9{
...
use eatdis_Spring_2015_2.dta, clear
foreach region in 3 4 5 6 7 8 9{
...
*3c) Combine the data for each year into one dataset for all years per outcome/season.
cd "projectnumber\analysis\datasets\comparisons\region"
clear
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach season in Spring Autumn{
		use "`outcome'_`season'_2008_allregions.dta", clear
			foreach num of numlist 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019{
		append using "`outcome'_`season'_`num'_allregions.dta"
			}
		save "`outcome'_`season'_allyearsregions.dta", replace		
...
*4. Create extra variables needed for regression analyses
************************************************************
...
*Save 
save "`outcome'_Spring_regions_easter.dta", replace
...
cd "projectnumber\analysis\datasets\comparisons\region"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach dataset in Spring_regions_easter Autumn_allyearsregions{
...
save "`outcome'_`dataset'2.dta", replace
