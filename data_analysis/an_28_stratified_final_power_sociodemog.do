*an_28_stratified_final_power_sociodemog


*Calculating the power of our one week regressions, stratified by sociodemographics, using the standard errors.
*****************************************************************************************************************************
...
**********
clear
foreach mod in gender age imd sbp dbp bmi alc smok{
...
order outcome season mod cat irr1 lci1 uci1 stderr1
 save "projectnumber\analysis\power_calcs\stratified_final\sociodemog/`outcome'_Autumn_`mod'_`category'.dta", replace
 }
		}
}
}
...
replace suff_powered = . if stderr1 ==.
order outcome season mod cat irr1 lci1 uci1 stderr1
 save "projectnumber\analysis\power_calcs\stratified_final\sociodemog/`outcome'_Spring_`mod'_`category'.dta", replace
 }
		}
}
}
...
*1c) Append all files.
clear
cd "projectnumber\analysis\power_calcs\stratified_final\sociodemog"
ssc install fs
fs "*.dta*"
foreach f in `r(files)' {
	append using `f'
}
	export excel using "power_stratified_sociodemog.xlsx", firstrow(variables) replace