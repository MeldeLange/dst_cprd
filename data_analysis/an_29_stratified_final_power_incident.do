*an_29_stratified_final_power_sociodemog

*Calculating the power of our one week regressions, stratified by incident vs prevalent case, using the standard errors.
...
order outcome season mod cat irr1 lci1 uci1 stderr1
 save "projectnumber\analysis\power_calcs\stratified_final\incident/`outcome'_Autumn_`mod'_`category'.dta", replace
 }
		}
}
}
...
replace suff_powered = . if stderr1 ==.
order outcome season mod cat irr1 lci1 uci1 stderr1
 save "projectnumber\analysis\power_calcs\stratified_final\incident/`outcome'_Spring_`mod'_`category'.dta", replace
 }
		}
}
}
...
*1c) Append all files.
clear
cd "projectnumber\analysis\power_calcs\stratified_final\incident"
ssc install fs
fs "*.dta*"
