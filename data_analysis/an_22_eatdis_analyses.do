*Analysis 22: Re-do eating disorders analyses to include years/regions with no events
**************************************************************************************
...
save eatdis_Spring_2013_4.dta, replace
...
foreach region in 2 3 4 5 6 7 8 9{
	append using "eatdis_Spring_2013_`region'.dta"
		...
		save "eatdis_Spring_2013_allregions.dta", replace	
...
foreach region in 2 3 4 5 6 7 8 9{
	append using "eatdis_Spring_2015_`region'.dta"
		...
		save "eatdis_Spring_2015_allregions.dta", replace	
...
foreach num of numlist 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019{
		...
		save "eatdis_Spring_allyearsregions.dta", replace	
...
*Save 
save "eatdis_Spring_regions_easter.dta", replace


*****************************

* Create the extra variables we need for the week comparisons

cd "projectnumber\analysis\datasets\comparisons\region"
use eatdis_Spring_regions_easter.dta, clear
...
*twowk_after
gen twowk_after =0
replace twowk_after = 1 if day == 29 | day == 30 | day == 31 | day == 32 | day == 33 | day == 34 | day == 35 | day == 36 | day == 37 | day == 38 | day == 39 | day == 40 | day == 41 | day == 42

*fourwk_after
gen fourwk_after =0
replace fourwk_after = 1 if day == 29 | day == 30 | day == 31 | day == 32 | day == 33 | day == 34 | day == 35 | day == 36 | day == 37 | day == 38 | day == 39 | day == 40 | day == 41 | day == 42 | day == 43 | day == 44 | day == 45 | day == 46 | day == 47 | day == 48 | day == 49 | day == 50 | day == 51 | day == 52 | day == 53 | day == 54 | day == 55 | day == 56

*wkdays_onewkafter

gen wkdays_onewkafter = .
replace wkdays_onewkafter = 1 if day == 30 | day == 31 | day == 32 | day == 33 | day == 34 
replace wkdays_onewkafter = 0 if day == 2 |day == 3 | day == 4 | day == 5 | day == 6 |day == 9 |day == 10 |day == 11 |day == 12 |day == 13 | day == 16 | day == 17 | day == 18 | day == 19 | day == 20 | day == 23 | day == 24 | day == 25 | day == 26 | day == 27| day == 37 |day == 38 |day == 39 |day == 40 |day == 41 | day == 44 | day == 45 | day == 46 | day == 47 | day == 48 | day == 51 | day == 52 | day == 53 |day == 54 |day == 55



*Create indicator variables for the individual days in the week after the clock change.
gen sun_after =.
replace sun_after = 0 if days == 1
replace sun_after = 1 if day ==29

gen mon_after =.
replace mon_after = 0 if days == 2
replace mon_after = 1 if day == 30

gen tues_after =.
replace tues_after = 0 if days == 3
replace tues_after = 1 if day == 31

gen wed_after =.
replace wed_after = 0 if days == 4
replace wed_after = 1 if day == 32

gen thurs_after =.
replace thurs_after = 0 if days == 5
replace thurs_after = 1 if day == 33

gen fri_after =.
replace fri_after = 0 if days == 6
replace fri_after = 1 if day == 34

gen sat_after =.
replace sat_after = 0 if days == 7
replace sat_after = 1 if day == 35

save eatdis_Spring_regions_easter2.dta, replace


******************************************************************************************

*3. Regression analysis comparing the number of events in the 1, 2 and 4 weeks, and Monday-Friday after the clock changes to all other weeks.
***************************************************************************************************************************************************
...
*Open log
log using "projectnumber\analysis\logs/eatdis_weeks.log", replace


cd "projectnumber\analysis\datasets\comparisons\region"

use "eatdis_Spring_regions_easter2.dta", clear
...
estout model1 using eatdis_Spring_regionsadj_oneweek.txt, eform replace cells("b(label(IRR) fmt(%9.2f)) ci(label(95% CI) fmt(%9.3f)) p(label(p value) fmt(%9.4f))") label stats(N, labels("n") fmt(%9.0g))
*Import regression results & save as excel spreadsheet
import delimited using eatdis_Spring_regionsadj_oneweek.txt, clear
export excel using "projectnumber\analysis\regressions\region\adjusted/eatdis_Spring_regionsadj_oneweek.xlsx", replace 
...
*twowk_after
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: Two weeks after"
eststo model1:nbreg count i.twowk_after i.days i.easter_wkend i.region, irr vce(cluster region) 
matrix list r(table) // displays results inc the full p value.
estout model1 using eatdis_Spring_regionsadj_twoweek.txt, eform replace cells("b(label(IRR) fmt(%9.2f)) ci(label(95% CI) fmt(%9.3f)) p(label(p value) fmt(%9.4f))") label stats(N, labels("n") fmt(%9.0g))
*Import regression results & save as excel spreadsheet
import delimited using eatdis_Spring_regionsadj_twoweek.txt, clear
export excel using "projectnumber\analysis\regressions\region\adjusted/eatdis_Spring_regionsadj_twoweek.xlsx", replace 
...
*fourwk_after
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: Four weeks after"
eststo model1:nbreg count i.fourwk_after i.days i.easter_wkend i.region, irr vce(cluster region) 
matrix list r(table) // displays results inc the full p value.
estout model1 using eatdis_Spring_regionsadj_fourweek.txt, eform replace cells("b(label(IRR) fmt(%9.2f)) ci(label(95% CI) fmt(%9.3f)) p(label(p value) fmt(%9.4f))") label stats(N, labels("n") fmt(%9.0g))
*Import regression results & save as excel spreadsheet
import delimited using eatdis_Spring_regionsadj_fourweek.txt, clear
export excel using "projectnumber\analysis\regressions\region\adjusted/eatdis_Spring_regionsadj_fourweek.xlsx", replace 
...
*wkdays_onewkafter
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: Weekdays in the week after"
eststo model1:nbreg count i.wkdays_onewkafter i.days i.easter_wkend i.region, irr vce(cluster region) 
matrix list r(table) // displays results inc the full p value.
estout model1 using eatdis_Spring_regionsadj_weekdays.txt, eform replace cells("b(label(IRR) fmt(%9.2f)) ci(label(95% CI) fmt(%9.3f)) p(label(p value) fmt(%9.4f))") label stats(N, labels("n") fmt(%9.0g))
*Import regression results & save as excel spreadsheet
import delimited using eatdis_Spring_regionsadj_weekdays.txt, clear
export excel using "projectnumber\analysis\regressions\region\adjusted/eatdis_Spring_regionsadj_weekdays.xlsx", replace 


**************************

*Get the total number of events included in each of the 1,2 & 4 weeks and Mon-Fri analyses 

*Spring - - 1,2 & 4 week analyses (all use all days of data)
cd "projectnumber\analysis\datasets\comparisons\region"
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: weeks"
total(count)


*Spring - Monday-Friday analyses (just use week days)
cd "projectnumber\analysis\datasets\comparisons\region"
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: weekdays"
...
*4. Regression analysis comparing the number of events individual days in the week after the clock changes to all other week days. 
***********************************************************************************************************************************

*Open log
log using "projectnumber\analysis\logs/eatdis_days.log", replace
...
matrix list r(table) // displays results inc the full p value.
estout model1 using eatdis_Spring_regionsadj_`day'.txt, eform replace cells("b(label(IRR) fmt(%9.2f)) ci(label(95% CI) fmt(%9.3f)) p(label(p value) fmt(%9.4f))") label stats(N, labels("n") fmt(%9.0g))
*Import regression results & save as excel spreadsheet
import delimited using eatdis_Spring_regionsadj_`day'.txt, clear
export excel using "projectnumber\analysis\regressions\region\adjusted/eatdis_Spring_regionsadj_`day'.xlsx", replace 

	}

*Get the total number of events included in each day of the week analyses
cd "projectnumber\analysis\datasets\comparisons\region"
foreach day in sun mon tues wed thurs fri sat{
use "eatdis_Spring_regions_easter2.dta", clear
di "eatdis" "Spring: `day'"
