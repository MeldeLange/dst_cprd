*Analysis 18 - Extract datasets stratified by outcome subgroup (CVD only (GP & HES APC only))
*******************************************************************************************


*Varible: em_subgroup_cvd: 1"Atrial Fibrillation / Arrhythmia" 2"Myocardial Infarction" 3"Stroke / TIA" 4"Other"

*1. Create a cvd dataset of only GP & hospital inpatient data (as it was not possible to differentiate between subgroups in the A&E data)
cd "projectnumber\cprd_data\combined\combined_eventlists"
use eventlist_cvd_22.dta, clear
drop if em_subgroup_cvd ==.
tab code_type // we only have icd and med codes not aepatgroup codes.
save eventlist_cvd_gphesapc.dta, replace


*2.Loop to extract initial datasets
...
*3 b)Combine the years for each outcome/season/effect modifier category into one dataset
clear
foreach mod in subgroup{
