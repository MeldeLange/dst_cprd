*an_stratified_final_sociodemographics

*Re-do stratified analyses to include years/regions/modifier categories with no events.

*1.SOCIODEMOGRAPHICS - loop to extract initial datasets.
*********************************************************************
*These variables have people with misisng data. levels of sociodemographics are 1 and 2 (and missing)
...
foreach mod in gender imd sbp dbp bmi alc smok{
local mlevels 1 2 . // 1 2 missing. 
foreach lev of local mlevels {
preserve
...
*Save datasets
save "projectnumber\analysis\stratified\final\datasets/`mod'/`outcome'_`season'_`year'_region`region'_`mod'`category'.dta", replace
restore
...
*********************************
*2a). SOCIODEMOGRAPHICS - Combine the regions for each year into one dataset

foreach mod in gender imd sbp dbp bmi alc smok{
	cd "projectnumber\analysis\stratified\final\datasets/`mod'"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep {
    foreach season in Spring Autumn {
        foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
           foreach category in  1 2 3 {
                ...
                if `"`filenames'"' != "" {
                    ...
                    save "projectnumber\analysis\stratified\final\datasets/`mod'\regions_combined/`outcome'_`season'_`year'_allregions_`mod'`category'.dta", replace
                }  
            }    
        }
    }
...
*2b)SOCIODEMOGRAPHICS - Combine the years for each outcome/season/effect modifier category into one dataset
clear
foreach mod in gender imd sbp dbp bmi alc smok{
...
*3. SOCIODEMOGRAPHICS - Create indicator variables needed for analysis
************************************************************************
