*an_stratified_final_incident

*Re-do stratified analyses to include years/regions/modifier categories with no events.

*1.INCIDENT - loop to extract initial incident vs prevalent datasets.
*********************************************************************
*incident has no missing data. levels of incident are 0 and 1.
*we don't split road traffic injuries by incident vs prevalent.
...
*Loop to extract each dataset
local incident_levels 0 1 // there is no missing data for incident
foreach incident of local incident_levels {
preserve
keep if em_incident_bin == `incident'	
local category = "`incident'"

if _N ==0 {
	
	use "projectnumber\analysis\datasets\comparisons\region/anx_Autumn_2008_1.dta", clear
	replace count =0
	replace year = `year'
...
*Save datasets
save "projectnumber\analysis\stratified\final\datasets\incident/`outcome'_`season'_`year'_region`region'_incident`category'.dta", replace
restore
...
*2a). INCIDENT - Combine the regions for each year into one dataset

	cd "projectnumber\analysis\stratified\final\datasets\incident"
foreach outcome in anx cvd dep eatdis psy selfharm sleep {
    foreach season in Spring Autumn {
        foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
           foreach category in 0 1{
                local filenames: dir "." files "`outcome'_`season'_`year'_region*_incident`category'.dta"
                clear
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\stratified\final\datasets\incident\regions_combined/`outcome'_`season'_`year'_allregions_incident`category'.dta", replace
                }  
            }    
        }
    }
...
*2b)INCIDENT - Combine the years for each outcome/season/effect modifier category into one dataset
clear
foreach mod in incident {
	cd "projectnumber\analysis\stratified\final\datasets/`mod'/regions_combined"
foreach outcome in anx cvd dep eatdis psy selfharm sleep{
	foreach season in Spring Autumn{
		 foreach category in 0 1{
                ...
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\stratified\final\datasets/`mod'/years_combined/`outcome'_`season'_allyearsregions_`mod'`category'.dta", replace
                }  
            }    
        }
    }
...
*3. INCIDENT - Create indicator variables needed for analysis
***************************************************
