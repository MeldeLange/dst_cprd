*Analysis 16 - Stratified regressions for the first week after the clock changes vs rest of the weeks, stratified by incident vs prevalent cases
***********************************************************************************************************************************************


**Open log
log using "projectnumber\analysis\logs/stratified_regressions_incident.log", replace // because having to use 'capture' command this doesn't log the regression outputs from part 1.
...
*3. Get exact p values for those that are 0.0000

*incident anxiety autumn no
cd "projectnumber\analysis\stratified\datasets/incident/years_combined"
use anx_Autumn_allyearsregions_incident0_2.dta, clear
nbreg count i.onewk_after i.days i.region, irr vce(cluster region) 
matrix list r(table)


*incident depression autumn no
cd "projectnumber\analysis\stratified\datasets/incident/years_combined"
use dep_Autumn_allyearsregions_incident0_2.dta, clear
nbreg count i.onewk_after i.days i.region, irr vce(cluster region) 
matrix list r(table)

*incident selfharm autumn yes
cd "projectnumber\analysis\stratified\datasets/incident/years_combined"
use selfharm_Autumn_allyearsregions_incident1_2.dta, clear
nbreg count i.onewk_after i.days i.region, irr vce(cluster region) 
matrix list r(table)
