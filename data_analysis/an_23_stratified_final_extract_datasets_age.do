...
*1.AGE - loop to extract initial age datasets.
************************************************
*age has no missing data. levels of age are 0 and 1.
...
local age_levels 0 1 // there is no missing data for age.
foreach age of local age_levels {
preserve
keep if em_age_bin == `age'	
local category = "`age'"
...
*Save datasets
save "projectnumber\analysis\stratified\final\datasets\age/`outcome'_`season'_`year'_region`region'_age`category'.dta", replace
restore
...
*2a). AGE - Combine the regions for each year into one dataset

	cd "projectnumber\analysis\stratified\final\datasets\age"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep {
    foreach season in Spring Autumn {
        foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
           foreach category in 0 1{
                local filenames: dir "." files "`outcome'_`season'_`year'_region*_age`category'.dta"
                clear
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\stratified\final\datasets\age\regions_combined/`outcome'_`season'_`year'_allregions_age`category'.dta", replace
                }  
            }    
        }
    }
...
*2b)AGE - Combine the years for each outcome/season/effect modifier category into one dataset
clear
foreach mod in age {
...
*3. AGE - Create indicator variables needed for analysis
***************************************************
