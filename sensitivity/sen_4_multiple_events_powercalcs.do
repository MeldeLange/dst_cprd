*Sensitivity 4: Power calcs (using standard errors) for Multiple Events Regressions
***********************************************************
...
save "projectnumber\analysis\power_calcs\multiple_events/`outcome'_Autumn.dta", replace
...
replace suff_powered = 1 if irr1 <= exp_lower | irr1 >= exp_upper
order outcome season irr1 lci1 uci1 stderr1
save "projectnumber\analysis\power_calcs\multiple_events/`outcome'_Spring.dta", replace
 }
...
*1c) Append all files.
clear
cd "projectnumber\analysis\power_calcs\multiple_events"
ssc install fs
fs "*.dta*"
foreach f in `r(files)' {
	append using `f'
}
	export excel using "power_multiple_events.xlsx", firstrow(variables) replace
