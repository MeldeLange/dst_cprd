*Sensitivity 2 - Extract datasets for initial sensitivity analysis to explore the effec of people having multiple events in an 8 week period surrounding a clock change.
************************************************************************************************************************************************************************

*Go back to raw eventlists & remove anyone with an extreme number of events (>=20) within each 8 week period.
*Then only keep one random event per person.
*Put the datasets back together again.
*Redo 1 week regressions to see if what we get is noticably different to the main results.
	

*Open log
log using "projectnumber\analysis\logs/multiple_events.log", replace


*Set working directory
cd "projectnumber\cprd_data\combined\combined_eventlists"

*1. Loop to take each 8 week period in our study (8 outcomes x 2 seasons x 12 years = 192 sub-datasets), exclude those with an extreme number of events (>=20) & only keep one random record per person.
...
egen obs_per_pat = count (patid), by(patid) // create variable with number of events for each person
drop if obs_per_pat >=20 // Exclude all observations for people with an extreme number of events (20 or more events in the 8 week period). 
set seed 608010 // set initial value of the random-number seed so always get the same random numbers.
gen random_num = runiform() // generate variable that is a random number between 0 and 1
by patid (random_num), sort: keep if _n ==1 // sort data by patient id and random number & keep just the first record for each person.
duplicates report (patid) // check we've only got one observation per patient
save "projectnumber\analysis\sensitivity\multiple_events/`outcome'_`year'_`season'_random.dta", replace
		}
	}
}
...
*2. Put datasets back together so that data for all years & seasons are back together.

cd "projectnumber\analysis\sensitivity\multiple_events"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	use "`outcome'_2008_Autumn_random.dta", clear
	append using "`outcome'_2008_Spring_random.dta" "`outcome'_2009_Autumn_random.dta" "`outcome'_2009_Spring_random.dta" "`outcome'_2010_Autumn_random.dta" "`outcome'_2010_Spring_random.dta" "`outcome'_2011_Autumn_random.dta" "`outcome'_2011_Spring_random.dta" "`outcome'_2012_Autumn_random.dta" "`outcome'_2012_Spring_random.dta" "`outcome'_2013_Autumn_random.dta"  "`outcome'_2013_Spring_random.dta" "`outcome'_2014_Autumn_random.dta" "`outcome'_2014_Spring_random.dta" "`outcome'_2015_Autumn_random.dta" "`outcome'_2015_Spring_random.dta" "`outcome'_2016_Autumn_random.dta" "`outcome'_2016_Spring_random.dta" "`outcome'_2017_Autumn_random.dta" "`outcome'_2017_Spring_random.dta" "`outcome'_2018_Autumn_random.dta" "`outcome'_2018_Spring_random.dta" "`outcome'_2019_Autumn_random.dta" "`outcome'_2019_Spring_random.dta" 
		save "projectnumber\analysis\sensitivity\multiple_events\appended/`outcome'_random.dta", replace		
...
*****************************************************************************************
*3. Extract the summary datasets needed, then combine year & region data for each outcome / season.
***********************************************************************************************************************************

*3a) Extact datasets by outcome, season, year and region.
cd "projectnumber\analysis\sensitivity\multiple_events\appended"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach season in Spring Autumn {
		foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
			foreach region in 1 2 3 4 5 6 7 8 9 {
				capture {
use `outcome'_random.dta, clear
keep if em_season == "`season'"	
keep if em_year == `year'
...
*Save datasets
save "projectnumber\analysis\sensitivity\multiple_events\datasets/`outcome'_`season'_`year'_region`region'.dta", replace
				}
	}
}
...
*4. Append the collapsed datasets so all years & regions together for each outcome, seasons separately.
clear
*4a) Combine the regions for each year into one dataset
	cd "projectnumber\analysis\sensitivity\multiple_events\datasets"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep {
    foreach season in Spring Autumn {
        foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
                local filenames: dir "." files "`outcome'_`season'_`year'_region*.dta"
                clear
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\sensitivity\multiple_events\datasets/regions_combined/`outcome'_`season'_`year'_allregions.dta", replace
                }  
            }    
        }
    }
...
*4 b)Combine the years for each outcome/season into one dataset
clear
...
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach season in Spring Autumn{
                ...
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\sensitivity\multiple_events\datasets\years_combined/`outcome'_`season'_allyearsregions.dta", replace
                }  
            }    
        }
...
*5. Create indicator variables needed for analysis
***************************************************
*5a) In Spring datasets create indicator variable for 5 days of the Easter weekend (inc Maundy Thursday)
clear
cd "projectnumber\analysis\sensitivity\multiple_events\datasets\years_combined"
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
		local filenames: dir "." files "`outcome'_Spring_allyearsregions.dta"
...
*************************************************************************
*5b) In all datasets generate days & week after the clock change variables 
************************************************************************
clear
cd "projectnumber\analysis\sensitivity\multiple_events\datasets\years_combined"
...
local filenames: dir "." files "`outcome'_`dataset'.dta"
 foreach file in `filenames'{
	 	use `file', clear
...
drop days
rename days2 days
save "`outcome'_`dataset'_2.dta", replace
}
		}
	}
