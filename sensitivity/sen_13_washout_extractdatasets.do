*Sensitivity 13 - Washout multiple events - - extract summary datasets.
**************************************************************************
...
use "`outcome'_washout.dta", clear
...
save "`outcome'_washout_2.dta", replace
}
...
merge 1:m pracid using "projectnumber\analysis\sensitivity\washout\culled_datasets/`outcome'_washout_2.dta"
drop if _merge ==1 
drop _merge
save "projectnumber\analysis\sensitivity\washout\culled_datasets/`outcome'_washout_3.dta", replace
}



*2. Extract the summary datasets needed, then combine year & region data for each outcome / season.
***********************************************************************************************************************************
	...
	foreach season in Spring Autumn {
		foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
			foreach region in 1 2 3 4 5 6 7 8 9 {
				capture {
use `outcome'_washout_3.dta, clear
...
*Save datasets
save "projectnumber\analysis\sensitivity\washout\sum_datasets/`outcome'_`season'_`year'_region`region'.dta", replace
				}
	}
}
...
*3. Append the collapsed datasets so all years & regions together for each outcome, seasons separately.
clear
    ...
    foreach season in Spring Autumn {
        foreach year in 2008 2009 2010 2011 2012 2013 2014 2015 2016 2017 2018 2019 {
                ...
                if `"`filenames'"' != "" {
                    ...
                    save "projectnumber\analysis\sensitivity\washout\sum_datasets/regions_combined/`outcome'_`season'_`year'_allregions.dta", replace
                }  
            }    
        }
    }
...
*3 b)Combine the years for each outcome/season into one dataset
clear
...
foreach outcome in anx cvd dep eatdis psy rti selfharm sleep{
	foreach season in Spring Autumn{
                ...
                if `"`filenames'"' != "" {
                    append using `filenames'
                    save "projectnumber\analysis\sensitivity\washout\sum_datasets\years_combined/`outcome'_`season'_allyearsregions.dta", replace
                }  
            }    
        }
...
*************************************************************************
*4b) In all datasets generate days & week after the clock change variables 
************************************************************************
clear
cd "projectnumber\analysis\sensitivity\washout\sum_datasets\years_combined"
